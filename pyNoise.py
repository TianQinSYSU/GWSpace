...
if __name__ == "__main__":
    freq  = np.logspace(-5,0,1001)
    la = LISANoise()
    lisa_sa, lisa_sp = la.noises(freq)
    
    tq = TianQinNoise()
    tq_sa, tq_sp = tq.noises(freq)
    
    LA, LE, LT = noise_AET(freq, lisa_sa, lisa_sp, la.armL)
    TA, TE, TT = noise_AET(freq, tq_sa, tq_sp, tq.armL)

    LX, LXY = noise_XYZ(freq, lisa_sa, lisa_sp, la.armL)
    TX, TXY = noise_XYZ(freq, tq_sa, tq_sp, tq.armL)
    #=======================================
    plt.figure()
    plt.title("Sensitivity of TianQin and LISA---with displacement")
    
    plt.loglog(freq, la.sensitivity(freq), label="LISA")
    plt.loglog(freq, tq.sensitivity(freq), label="TianQin")
    ...
    plt.loglog(freq, lisa_sa, ':r', label="LISA Sa")
    plt.loglog(freq, lisa_sp, '--r', label="LISA Sp")
    
    plt.plot(freq, tq_sa, "-b", label="TianQin Sa")
    plt.plot(freq, tq_sp, "--b", label="TianQin Sp")

    plt.loglog(freq, M_spm(freq), '-g', label="Marsat Sa")
    plt.loglog(freq, M_sop(freq), '--g', label="Marsat Sp")
    ...
    plt.loglog(freq, LA, "-", label="LISA AE")
    plt.loglog(freq, LT, "-.", label="LISA T")
    
    plt.loglog(freq, TA, "--", label="TianQin AE")
    plt.loglog(freq, TT, "-.", label="TianQin T")
   ...
   #=======================================
    ...
    plt.loglog(freq, LX, "-", label="LISA X")
    plt.loglog(freq, LXY, "-.", label="LISA XY")
    
    plt.loglog(freq, TX, "--", label="TianQin X")
    plt.loglog(freq, TXY, "-.", label="TianQin XY")
    ...
    #**************************************
    la = LISANoise()
    lisa_sa, lisa_sp = la.noises(freq, unit="displacement")
    
    tq = TianQinNoise()
    tq_sa, tq_sp = tq.noises(freq, unit="displacement")
