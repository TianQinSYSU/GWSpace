...
G = 6.67408e-11  # Newtonian constant of gravitation [m^3.kg^-1.s^-2] (CODATA 2014)
...
# MsunKG = GMsun/G
# MsunKG = 1.988475e30        # Solar mass [kg] (Luzum et al., The IAU 2009 system of astronomical constants: the report of the IAU working group on numerical standards for Fundamental Astronomy, Celestial Mechanics and Dynamical Astronomy, 2011)
ua = 149597870700.  # Astronomical unit [m](resolution B2 of IAU2012)
YRSID_SI = 31558149.763545600  # sidereal year [sec] (http://hpiers.obspm.fr/eop-pc/models/constants.html)
...
def tf(freq, phase, t0):
    ...
    # The 'scipy' is an HPC package, and is difficult to implement Python JIT by 'numba'.

    # The 'scipy.interpolate.InterpolatedUnivariateSpline.derivative' can only use in (size > 3).
    # When (3 >= size > 1), I use the 'numpy.gradient' to calculate the derivation.
    # Maybe it's not necessary, because I don't expect a high likelihood/SNR ratio in this case.
    if phase.size > 3:
        tfspline = Spline(freq,
                          1/(2*pi)*(phase-phase[0])).derivative()  # get rid of possibly huge constant in the phase
...
def generate_TQ_TDI_FDwaveform(para, Tobs, frequency=None):
    ...
    @ para      : (numpy.array[float]) an 11-dimensional parameter array for BBH.
