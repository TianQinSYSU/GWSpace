...
@njit()
def Freq_Ins(Tobs,tc,Mc):
    if (Tobs >= tc):
        raise ValueError('This function (Freq_Ins) only can be used in inspiral signal!')
    Mc_T = Mc * MTsun
    return 1/(8*pi) * ((tc-Tobs)/5)**(-3/8) * Mc_T**(-5/8)

@njit()
def freq_bound(Tobs,tc,Mc,eta):
    if (Tobs >= tc):
        f_max = Freq_Max(Mc,eta)
    else:
        f_max = Freq_Ins(Tobs,tc,Mc)
    if (f_max > 1.0):
        f_max = 1.0

    f_min = Freq_Ins(0,tc,Mc)
    if (f_min < 1e-4):
        f_min = 1e-4
    
    return f_max, f_min

# 'pyIMRPhenomD' can calculate the 'TimeF' directly. So I remove this function.
# def TimeF(freq, phase, tc):
...
#         tf_vec = tfspline(freq) + tc
#     elif (phase.size > 1):
#         tf_vec = np.gradient(1/(2*pi)*(phase - phase[0])) / np.gradient(freq) + tc
#     elif (phase.size == 1):
#         tf_vec = tc
#     else:
...
def TQ_orbit_func(timef):
    x1,y1,z1, x2,y2,z2, x3,y3,z3 = TQ_orbit(timef)
...
@njit()
def AET_response(wf, tc, res_vec, k_vec, p0_earth):
    freq, amp, phase, timef = wf
    ...
    ampphasefactor = amp * np.exp(1j*(phase + phaseRdelay)) * np.exp(2j*pi*freq*tc)
...
def generate_TQ_AmpPhase(para_list, Tobs, NF=1024):
    Mc = para_list[0]
    eta = para_list[1]
    ...
    # deltaF = 1 / Tobs
    f_max, f_min = freq_bound(Tobs,tc,Mc,eta)

    if (f_min > f_max): # This can happen when f_min > 1 Hz or f_max < 10^-4 Hz
        return [], [], [], []
    ...
    h22 = IMRPhenomDGenerateh22FDAmpPhase(h22,freq,phic,fRef_in,m1_SI,m2_SI,chi1,chi2,distance)

    amp = h22.amp
    phase = h22.phase
    timef = h22.time
    return freq, amp, phase, timef
...
def generate_TQ_response(para_list, Tobs, wf):
    ...
    p0_earth, U_vec, p_vec = TQ_orbit_func(timef)
    res_a, res_e, res_t = TQ_FDresponse(freq, U_vec, p_vec, k_vec, P22_mat)
    res_vec = np.array([res_a, res_e, res_t])

    Achannel, Echannel, Tchannel = AET_response(wf, tc, res_vec, k_vec, p0_earth)
...
def generate_TQ_TDI_FDwaveform(para, Tobs, frequency=None):
    ...
    @ para      : (numpy.array[float]) a 11-dimensional parameter array for BBH.
                ...
                [5] r$t_c$     : merge time [second]; 
                [6] r$\phi_c$  : merge phase (0,2*pi) [rad]; 
    ...
    freq, amp, phase, timef = generate_TQ_AmpPhase(para, Tobs)
    ...
    if (len(freq) > 1):
        ...
        timef_spline = interp1d(freq,timef,bounds_error=False,fill_value=0)
        ...
        timef_idx = timef_spline(freq_idx)
    else: 
        idx = np.full(Nf, False, dtype=bool)
        return frequency, signalA, signalE, signalT, idx

    wf = np.array([freq_idx, amp_idx, phase_idx, timef_idx])
