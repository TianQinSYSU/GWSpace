...
def k_u(theta, phi, alpha, beta, det, u):
    ...
    if det == 'TQ2':
        ...
        if u == 'u2':
            return (-np.cos(theta)*(np.cos(alpha+beta)+np.sin(alpha+beta+np.pi/6))+np.cos(phi)*np.sin(theta)*(
                        -np.cos(alpha+beta+np.pi/6)+np.sin(alpha+beta)))/np.sqrt(3)
        if u == 'u3':
            return (np.cos(theta)*(-np.cos(alpha+beta)+np.sin(alpha+beta-np.pi/6))+np.cos(phi)*np.sin(theta)*(
                        np.cos(alpha+beta-np.pi/6)+np.sin(alpha+beta)))/np.sqrt(3)
...
def k_dx11(theta, phi, alpha, beta):
    return (np.cos(theta)*np.sin(alpha+beta+np.pi/6)-(
                np.cos(phi)*(np.cos(alpha+np.pi/6)-np.cos(alpha+beta+np.pi/6))+np.sin(phi)*np.sin(
            alpha+np.pi/6))*np.sin(theta))/np.sqrt(3)
...
def F_ab(theta, phi, alpha, beta, det, pol, u):
    if det == 'TQ':
        if pol == 'p':
            ...
            if u == 'u2':
                return (-2+2*np.cos(2*theta)-np.cos(1/3*(-6*alpha+6*phi+np.pi+6*theta))-6*np.sin(
                    2*alpha-2*phi+np.pi/6)-np.sin(2*alpha-2*phi+np.pi/6+2*theta))/16
            if u == 'u3':
                return (-2+2*np.cos(2*theta)+6*np.sin(2*alpha-2*phi-np.pi/6)+np.sin(
                    2*alpha-2*phi-np.pi/6-2*theta)+np.sin(2*alpha-2*phi-np.pi/6+2*theta))/16
        if pol == 'c':
            if u == 'u1':
    ...
    if det == 'TQ2':
        if pol == 'p':
            if u == 'u1':
                return (np.cos(alpha+beta)**2*(np.cos(phi)**2*np.cos(theta)**2-np.sin(phi)**2)+np.sin(
                    alpha+beta)**2*np.sin(theta)**2-np.cos(alpha+beta)*np.cos(phi)*np.sin(alpha+beta)*np.sin(2*theta))/2
            if u == 'u2':
                return (4*(np.cos(phi)**2*np.cos(theta)**2-np.sin(phi)**2)*np.sin(alpha+beta-np.pi/6)**2+4*np.cos(
                    alpha+beta-np.pi/6)**2*np.sin(theta)**2+np.sin(2*phi)*np.sin(2*(alpha+beta-np.pi/6))*np.sin(
                    2*theta)/np.sin(phi))/8
            if u == 'u3':
                return ((np.cos(phi)**2*np.cos(theta)**2-np.sin(phi)**2)*np.sin(alpha+beta+np.pi/6)**2+np.cos(
                    alpha+beta+np.pi/6)**2*np.sin(theta)**2+np.cos(phi)*np.cos(alpha+beta+np.pi/6)*np.sin(
                    alpha+beta+np.pi/6)*np.sin(2*theta))/2
        if pol == 'c':
            if u == 'u1':
                return np.cos(alpha+beta)*(
                            np.cos(alpha+beta)*np.cos(theta)*np.sin(2*phi)-2*np.sin(alpha+beta)*np.sin(phi)*np.sin(
                        theta))/2
            if u == 'u2':
                return np.sin(alpha+beta-np.pi/6)*(
                            np.cos(theta)*np.sin(2*phi)*np.sin(alpha+beta-np.pi/6)+2*np.cos(alpha+beta-np.pi/6)*np.sin(
                        phi)*np.sin(theta))/2
            if u == 'u3':
                return np.sin(alpha+beta+np.pi/6)*(
                            np.cos(theta)*np.sin(2*phi)*np.sin(alpha+beta+np.pi/6)+2*np.cos(alpha+beta+np.pi/6)*np.sin(
...
def FP_M(f, theta, phi, alpha, beta, det, pol, u1, u2, direc1, direc2, td):
    return F_P(f, theta, phi, alpha, beta, det, pol, u1, direc1, td)-F_P(f, theta, phi, alpha, beta, det, pol, u2,
                                                                         direc2, td)
...
def FP_M2(f, theta, phi, alpha, beta, det, pol):
    ...
    return (FP_M(f, theta, phi, alpha, beta, det, pol, 'u1', 'u2', 1, 1, 0)+2*FP_M(f, theta, phi, alpha, beta, det, pol,
                                                                                   'u2', 'u3', -1, -1, 0)*ex)/np.sqrt(3)
...
for i in range(nn**2):
    int_orf_lm[i] = np.array(
        [int_theta[int(i/nn)], int_theta[int(i/nn+1)], int_phi[int(i) % nn], int_phi[int(int(i) % nn+1)]])
...
def ORF_lm(f, alpha, beta, det1, det2, chn1, chn2, l, m):
    ...
    if f < 0.9:
        tempr = dblquad(lambda phi, theta: np.sin(theta)*(inte_ORF_lm(f, theta, phi, alpha, beta)).real, 0, np.pi,
                        lambda theta: 0, lambda theta: 2*np.pi)[0]/(8*np.pi)
        tempi = dblquad(lambda phi, theta: np.sin(theta)*(inte_ORF_lm(f, theta, phi, alpha, beta)).imag, 0, np.pi,
                        lambda theta: 0, lambda theta: 2*np.pi)[0]/(8*np.pi)
        return tempr+tempi*1j
    else:
        def DBint(args):
            a1, a2, b1, b2 = args
            int_r = dblquad(lambda phi, theta: np.sin(theta)*(inte_ORF_lm(f, theta, phi, alpha, beta)).real, a1, a2,
                            lambda theta: b1, lambda theta: b2)[0]/(8*np.pi)
            int_i = dblquad(lambda phi, theta: np.sin(theta)*(inte_ORF_lm(f, theta, phi, alpha, beta)).imag, a1, a2,
                            lambda theta: b1, lambda theta: b2)[0]/(8*np.pi)
...
def ORF_l_ave(f, det1, det2, chn1, chn2, l):
    if det1 == det2:
        ...
        return np.sqrt(
            integrate.quad(lambda alpha: ORF_l(f, alpha, det1, det2, chn1, chn2, l)**2, 0, 2*np.pi)[0]/(2*np.pi))
...
def ORF_lm(f, alpha, beta, det1, det2, chn1, chn2, l, m):
    ...
    tempr = \
    dblquad(lambda phi, theta: np.sin(theta)*(inte_ORF(f, theta, phi, alpha, beta)).real, 0, np.pi, lambda theta: 0,
            lambda theta: 2*np.pi)[0]/(8*np.pi)
    tempi = \
    dblquad(lambda phi, theta: np.sin(theta)*(inte_ORF(f, theta, phi, alpha, beta)).imag, 0, np.pi, lambda theta: 0,
            lambda theta: 2*np.pi)[0]/(8*np.pi)
    return tempr+tempi*1j
